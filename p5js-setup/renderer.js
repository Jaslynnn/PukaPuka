...
function drawDotFilter() {
  ...
  // Touch tint: vivid magenta so contact is unmistakable
  const TR = 255, TG = 0, TB = 200;
...
function drawAudioHUD() {
  ...
  for (let i = MAX_TARGETS - 1; i >= 0; i--) {
    ...
    rows.forEach(({ label, value, display }) => {
      noStroke(); fill(140); textSize(10); textAlign(LEFT, CENTER);
